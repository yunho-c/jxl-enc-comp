...
def _profile_case(
    *,
    ...
    encoded_path = encoded_dir / f"{_case_id(image.image_id, encoder_name, mode, effort, distance)}.jxl"
    ...
    result.encode_seconds_per_mp = (
        encode_result.elapsed_seconds / image.megapixels if image.megapixels > 0 else None
    )
    result.command = encode_result.command_text
    if not encode_result.ok:
        result.status = "failed"
        result.reason = "encode failed"
        result.stderr = encode_result.stderr.strip()[-4000:]
        return result
